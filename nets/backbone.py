...
class MobileConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        super().__init__()
        self.cconv   = nn.Conv2d(c1, c1, k, s, autopad(k, p, d), groups=c1, dilation=d, bias=False) # 逐通道卷积
        self.pconv = nn.Conv2d(c1, c2, 1, 1, 0, groups=g, dilation=d, bias=False) # 点卷积
        self.cbn     = nn.BatchNorm2d(c1, eps=0.001, momentum=0.03, affine=True, track_running_stats=True)
        self.pbn = nn.BatchNorm2d(c2, eps=0.001, momentum=0.03, affine=True, track_running_stats=True)
        #self.act    = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()
        self.act = nn.LeakyReLU()

    def forward(self, x):
        x = self.act(self.cbn(self.cconv(x)))
        x = self.act(self.pbn(self.pconv(x)))
        return x
...
class MobileBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = MobileConv(c1, c_, k[0], 1)
        self.cv2 = MobileConv(c_, c2, k[1], 1, g=g)
        self.add = shortcut and c1 == c2
...
class MobileC2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__()
        self.c      = int(c2 * e)
        self.cv1    = MobileConv(c1, 2 * self.c, 1, 1)
        self.cv2    = MobileConv((2 + n) * self.c, c2, 1)
        self.m      = nn.ModuleList(MobileBottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
