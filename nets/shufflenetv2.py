...
class ShuffleNetV2(nn.Module):
    def __init__(self,
        ...
        self.conv5 = nn.Sequential(
            nn.Conv2d(input_channels, int(output_channels), kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(output_channels),
            nn.ReLU(inplace=True)
        )
...
def shufflenet_v2_x0_5(pretrained, num_classes=1000):
    """
...
def shufflenet_v2_x1_0(pretrained, num_classes=1000):
    """
    ...
    model = ShuffleNetV2(stages_repeats=[4, 8, 4],
                         ...
                         num_classes=num_classes)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls['shufflenet_v2_x1_0'], model_dir='./model_data',
                                              progress=True)
        model.load_state_dict(state_dict, strict=False)
    return model


def shufflenet_v2_x1_5(pretrained, num_classes=1000):
    """
    Constructs a ShuffleNetV2 with 1.0x output channels, as described in
    `"ShuffleNet V2: Practical Guidelines for Efficient CNN Architecture Design"
    <https://arxiv.org/abs/1807.11164>`.
    ...
    model = ShuffleNetV2(stages_repeats=[4, 8, 4],
                         ...
                         num_classes=num_classes)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls['shufflenet_v2_x1_5'], model_dir='./model_data',
                                              progress=True)
        model.load_state_dict(state_dict, strict=False)
    return model


def shufflenet_v2_x2_0(pretrained, num_classes=1000):
    """
    Constructs a ShuffleNetV2 with 1.0x output channels, as described in
    `"ShuffleNet V2: Practical Guidelines for Efficient CNN Architecture Design"
    <https://arxiv.org/abs/1807.11164>`.
    ...
    model = ShuffleNetV2(stages_repeats=[4, 8, 4],
                         ...
                         num_classes=num_classes)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls['shufflenet_v2_x2_0'], model_dir='./model_data',
                                              progress=True)
        model.load_state_dict(state_dict, strict=False)
    return model
