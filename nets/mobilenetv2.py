...
class MobileNetV2(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(self.features[0:11])
        x2 = self.features[7:14](x1)
        x3 = self.features[14:18](x2)
        #print(self.features)
        #x = self.avgpool(x)
        #x = torch.flatten(x, 1)
        #x = self.classifier(x)
...
def mobilenet_v2(pretrained, num_classes: int = 1000):

    model = MobileNetV2(num_classes=num_classes, alpha=1.0, round_nearest=8)
    if pretrained:
